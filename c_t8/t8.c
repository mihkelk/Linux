...
#include <ncurses.h>  //Vajalik PDCurses kasutamiseks.
int x, y, z;
...
int maailm() {
	...
	for (z = 49; z < 51; z++) {
		...
		//cz[9] = sprintf(cz, "%d", z); //sprintf(target string, "%d", source_int))
...
int joonistamine() {
	...
	for (y = py - jsuurus; y < (py + jsuurus); y++) {
		...
		for (x = px - jsuurus; x < (px + jsuurus); x++) {
			...
			case '0': //Tühjus
				addch(ACS_BLOCK | COLOR_PAIR(6));
				// Sümbolid laiendatud ascii tabelist pdcursesiga nimetused ACS_*
				break;
			case '1': //Sein
				addch(ACS_BLOCK | COLOR_PAIR(1));
				// Värve võetakse paaridena mis on defineeritud peafunktsioonis.
				break;
			case '2': //Muru
				addch(ACS_CKBOARD | COLOR_PAIR(2));
				break;
			case '3': //Mängija
				addch('\1' | COLOR_PAIR(4) | A_ALTCHARSET);
				// Altcharset vajalik naerunäeo näitamiseks.
				break;
			case '4': //Heledam muru
